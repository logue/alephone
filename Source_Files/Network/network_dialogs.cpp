...
public:
	...
	{
		vertical_placer *placer = new vertical_placer;
		placer->dual_add(new w_title(_SJIS("ネットワークゲームの募集")), m_dialog);
		...
		autogather_placer->dual_add(autogather_w->label(_SJIS("自動募集")), m_dialog);
		...
		w_button* play_button_w = new w_button(_SJIS("プレイ"));
		...
		chat_choice_placer->dual_add(chat_choice_w->label(_SJIS("チャット：")), m_dialog);
		...
		say_placer->dual_add(chatentry_w->label(_SJIS("発言：")), m_dialog);
...
public:
	SdlJoinDialog() : m_tabs(0)
	{
		...
		vertical_placer *placer = new vertical_placer;
		placer->dual_add(new w_title(_SJIS("ネットワークゲームに参加")), m_dialog);
		placer->add(new w_spacer(), true);
...
public:
	SdlSetupNetgameDialog ()
	{
		...
		placer->dual_add(new w_title(_SJIS("ネットワークゲームのセットアップ")), m_dialog);
		placer->add(new w_spacer(), true);
		...
		player_table->dual_add_row(new w_static_text(_SJIS("外観")), m_dialog);
		w_text_entry *name_w = new w_text_entry (PREFERENCES_NAME_LENGTH, "");
		name_w->enable_mac_roman_input();

		player_table->dual_add(name_w->label(_SJIS("名前")), m_dialog);
		player_table->dual_add(name_w, m_dialog);

		w_player_color *pcolor_w = new w_player_color (player_preferences->color);
		player_table->dual_add(pcolor_w->label(_SJIS("色")), m_dialog);
		player_table->dual_add(pcolor_w, m_dialog);

		w_player_color *tcolor_w = new w_player_color (player_preferences->team);
		player_table->dual_add(tcolor_w->label(_SJIS("チーム")), m_dialog);
		player_table->dual_add(tcolor_w, m_dialog);

		right_placer->dual_add(new w_static_text(_SJIS("ネットワーク")), m_dialog);
		...
		network_table->dual_add(advertise_on_metaserver_w->label(_SJIS("インターネットにゲーム募集を表示")), m_dialog);

		w_toggle *use_upnp_w = new w_toggle (true);
		network_table->dual_add(use_upnp_w, m_dialog);
		network_table->dual_add(use_upnp_w->label(_SJIS("UPnPルーターの設定")), m_dialog);
		...
		network_table->dual_add(realtime_audio_w->label(_SJIS("マイクを有効化")), m_dialog);
		...
		latency_placer->dual_add(latency_tolerance_w->label(_SJIS("許容遅延誤差")), m_dialog);
		...
		player_table->dual_add_row(new w_static_text(_SJIS("ゲーム")), m_dialog);
		...
		w_file_chooser* map_w = new w_file_chooser (_SJIS("マップ選択"), _typecode_scenario);
#ifndef MAC_APP_STORE
		player_table->dual_add(map_w->label(_SJIS("マップ")), m_dialog);
		player_table->dual_add(map_w, m_dialog);
#endif

		w_select_popup* entry_point_w = new w_select_popup ();
		player_table->dual_add(entry_point_w->label(_SJIS("レベル")), m_dialog);
		player_table->dual_add(entry_point_w, m_dialog);

		w_select_popup* game_type_w = new w_select_popup ();
		player_table->dual_add(game_type_w->label(_SJIS("ゲームの種類")), m_dialog);
		player_table->dual_add(game_type_w, m_dialog);

		w_select *diff_w = new w_select(network_preferences->difficulty_level, NULL);
		player_table->dual_add(diff_w->label(_SJIS("難易度")), m_dialog);
		...
		network_table->dual_add_row(new w_static_text(_SJIS("ネットスクリプト")), m_dialog);
#endif
		...
		network_table->dual_add(use_netscript_w->label(_SJIS("ネットスクリプトを使用")), m_dialog);
#endif

		w_file_chooser* choose_script_w = new w_file_chooser (_SJIS("スクリプトを使用"), _typecode_netscript);
#ifndef MAC_APP_STORE
		network_table->add(new w_spacer(), true);
		...
		options_table->dual_add_row(new w_static_text(_SJIS("オプション")), m_dialog);
		...
		options_table->dual_add(aliens_w->label(_SJIS("エイリアン")), m_dialog);
		...
		options_table->dual_add(live_w->label(_SJIS("ライブ虐殺レポート")), m_dialog);
		...
		options_table->dual_add(teams_w->label(_SJIS("チーム")), m_dialog);

		w_toggle *drop_w = new w_toggle(!(network_preferences->game_options & _burn_items_on_death));
		options_table->dual_add(drop_w, m_dialog);
		options_table->dual_add(drop_w->label(_SJIS("死んだプレイヤーはアイテムを落とす")), m_dialog);
		...
		options_table->dual_add(sensor_w->label(_SJIS("モーションセンサーを無効化")), m_dialog);
		...
		options_table->dual_add(pen_die_w->label(_SJIS("死亡ペナルティ(10秒)")), m_dialog);
		...
		options_table->dual_add(pen_sui_w->label(_SJIS("自殺ペナルティ(15秒)")), m_dialog);

		left_placer->add(options_table, true);

		network_table->add_row(new w_spacer(), true);
		network_table->dual_add_row(new w_static_text(_SJIS("チート／その他")), m_dialog);
		w_toggle *zoom_w = new w_toggle(true);
		network_table->dual_add(zoom_w, m_dialog);
		network_table->dual_add(zoom_w->label(_SJIS("ズームを許可")), m_dialog);
	
		w_toggle *crosshairs_w = new w_toggle(true);
		network_table->dual_add(crosshairs_w, m_dialog);
		network_table->dual_add(crosshairs_w->label(_SJIS("クロスヘアーを許可")), m_dialog);
	
		w_toggle *overlay_w = new w_toggle(true);
		network_table->dual_add(overlay_w, m_dialog);
		network_table->dual_add(overlay_w->label(_SJIS("オーバーレイマップを許可")), m_dialog);
		
		w_toggle *lara_croft_w = new w_toggle(true);
		network_table->dual_add(lara_croft_w, m_dialog);
		network_table->dual_add(lara_croft_w->label(_SJIS("追跡カメラを許可")), m_dialog);

		w_toggle *carnage_messages_w = new w_toggle(true);
		network_table->dual_add(carnage_messages_w, m_dialog);
		network_table->dual_add(carnage_messages_w->label(_SJIS("殺傷メッセージを許可")), m_dialog);

		w_toggle *saving_level_w = new w_toggle(true);
#ifndef MAC_APP_STORE
		network_table->dual_add(saving_level_w, m_dialog);
		network_table->dual_add(saving_level_w->label(_SJIS("レベルの保存を許可")), m_dialog);
#endif

		right_placer->add(new w_spacer(), true);
		right_placer->dual_add(new w_static_text(_SJIS("期間")), m_dialog);
		...
		limits_table->dual_add(endcondition_w->label(_SJIS("ゲームの終了条件")), m_dialog);
		...
		limits_table->dual_add(timelimit_w->label(_SJIS("時間制限（分）")), m_dialog);
		...
		limits_table->dual_add(scorelimit_w->label(_SJIS("殺傷・点数制限")), m_dialog);
...
network_gather(bool) {
    ...
    d.add(new w_title(_SJIS("マイクのテスト")));

    w_toggle*   onoff_w = new w_toggle(_SJIS("アクティブ"), 0);
    onoff_w->set_selection_changed_callback(respond_to_microphone_toggle);
    d.add(onoff_w);

    d.add(new w_button(_SJIS("完了"), dialog_ok, &d));

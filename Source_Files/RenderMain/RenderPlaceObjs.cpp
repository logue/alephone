	...
	short base_node_count)
{
	...
	/* find the node weﾕd like to be in (that is, the node closest to the viewer of all the nodes
	...
	/* update the .node fields of all the objects weﾕre about to add to reflect their new
...
/* we once thought it would be a clever idea to use the transformed endpoints, but, not.  we
	now bail if we canﾕt find a way out of the polygon we are given; usually this happens
	when weﾕre moving along gridlines */
...
//				dprintf("%s: (#%d,#%d)==>(#%d,#%d) (by #%d)", cast_state==_casting_left ? "left" : "right", origin->x, origin->y, destination.x, destination.y, cast_state==_casting_left ? left_distance : right_distance);
		...
		do
		{
			...
			short state= _looking_for_first_nonzero_vertex; /* really: testing first vertex state (we donﾕt have zero vertices) */
			...
			do
			{
				...
				if (state!=NONE&&!vertex_index) polygon_index= state= NONE; /* we canﾕt find a way out; give up */
			}
			while (state!=NONE);
			...
			if (polygon_index!=NONE)
			{
				...
				/* canﾕt do above clipping (see note in change history) */
				...
				{
					/* if weﾕre above the viewer and going into a lower polygon or below the viewer and going
						into a higher polygon, donﾕt */
...
//					dprintf("  into polygon #%d", polygon_index);
					if (!TEST_RENDER_FLAG(polygon_index, _polygon_is_visible)) polygon_index= NONE; /* donﾕt have transformed data, donﾕt even try! */

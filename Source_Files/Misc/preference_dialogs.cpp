...
public:
	SdlOpenGLDialog (int theSelectedRenderer)
	{
		...
		general_table->dual_add(liq_w->label("液面を半透明化"), m_dialog);
		...
		general_table->dual_add(ephemera_w->label("スクリプトによるエフェクトのクオリティ"), m_dialog);

...
static const char *action_name[NUM_KEYS] = {
	"前進", "後退", "左を向く", "右を向く", "左にサイドステップ", "右にサイドステップ",
...
static void mouse_custom_dialog(void *arg)
{
	...
	table->dual_add(mouse_precision_w->label("武器エイミングのスナップ"), d);
...
static void controller_details_dialog(void *arg)
{
	...
	table->dual_add(sens_joy_w->label("エイミングの感度"), d);
...
static void controls_dialog(void *arg)
{
	...
	controller_options->dual_add(joystick_aiming_w->label("コントローラーのエイミング"), d);
